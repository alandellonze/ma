...
INSERT INTO album(band_id, "position", type, typecount, name, year, status, maType, maTypeCount, maname)
  ...
  SELECT
    ...
    (SELECT id FROM band WHERE name = band_name) AS band_id,
    position,
  ...
  FROM
    ...
    (SELECT
     ...
     FROM
       discographies) subQuery

  ORDER BY band_name, position;


-- clean imported data
UPDATE album SET type = 'FULLLENGTH' WHERE type = '';
UPDATE album SET maType = 'FULLLENGTH' WHERE maType = '';
UPDATE album SET maName = NULL WHERE maName = '';

UPDATE album SET maType = NULL WHERE type = maType;
UPDATE album SET maTypeCount = NULL WHERE type IS NOT null AND typeCount = maTypeCount AND name IS NOT null;
UPDATE album SET maName = NULL WHERE name = maName;
